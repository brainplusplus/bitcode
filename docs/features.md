...
| 71 | Data Migration System | ✅ | — | Laravel-style data migration/seeder system. Multi-format sources (JSON, CSV, XLSX, XML). `migrations/` folder per module with timestamped files (`YYYYMMDD_HHMMSS_name.json`). `ir_migration` tracking table with batch support. Up/down (rollback) support. Custom processors (script/process). Field mapping, defaults, upsert/skip/error conflict modes. CLI: `bitcode seed run/rollback/status/fresh/create`. Auto-runs on module install. 18 tests. | — |
