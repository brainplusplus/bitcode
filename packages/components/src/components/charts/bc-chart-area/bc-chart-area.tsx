...
@Component({ tag: 'bc-chart-area', styleUrl: 'bc-chart-area.css', shadow: false })
export class BcChartArea {
  ...
  private renderChart() {
    ...
    this.chart.setOption({
      ...
      xAxis: { type: 'category', data: parsed.map(d => d.name), boundaryGap: false },
      yAxis: { type: 'value' },
      series: [{ type: 'line', data: parsed.map(d => d.value), areaStyle: {}, smooth: true, itemStyle: { color: '#4f46e5' } }],
    });
