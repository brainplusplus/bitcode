...
export class BcFieldInteger {
  ...
  private handleInput(e: Event) {
    ...
    if (this._getValidateOn() === 'change') debounce(`validate-${this.name}`, () => this._runValidation(), 300);
  }
  ...
  private handleClear() {
    const oldValue = this.value;
    this.value = 0;
    this._fieldState = markDirty(this._fieldState, 0);
    this.lcFieldClear.emit({ name: this.name, oldValue });
    this.lcFieldChange.emit({ name: this.name, value: 0, oldValue });
    this._inputEl?.focus();
  }
  ...
  private async _runValidation(): Promise<ValidationResult> {
    ...
    const result = await validateFieldValue(this.value, {
      required: this.required, min: this.min || undefined, max: this.max || undefined,
