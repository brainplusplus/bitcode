...
@Component({ tag: 'bc-field-markdown', styleUrl: 'bc-field-markdown.css', shadow: false })
export class BcFieldMarkdown {
  ...
  render() {
    ...
    return (
      <div class={{ ...fieldClasses, 'bc-md-wrap': true }}>
        ...
        {!this.showPreview ? (
          <textarea ref={(el) => this._textareaEl = el} class="bc-md-editor" value={this.value} placeholder={this.placeholder} required={this.required} readOnly={this.readonly} disabled={this.disabled} maxLength={this.maxLength > 0 ? this.maxLength : undefined} onInput={(e: Event) => this.handleInput(e)} onFocus={() => this.handleFocus()} onBlur={() => this.handleBlur()}></textarea>
        ) : (
