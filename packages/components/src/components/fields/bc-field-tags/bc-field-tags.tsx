...
@Component({ tag: 'bc-field-tags', styleUrl: 'bc-field-tags.css', shadow: false })
export class BcFieldTags {
  ...
  private removeTag(val: string) {
    const old = this.value;
    this.value = JSON.stringify(this.getValues().filter(t => t !== val));
    this._fieldState = markDirty(this._fieldState, this.value);
    this.lcFieldChange.emit({ name: this.name, value: this.value, oldValue: old });
  }
  ...
  render() {
    ...
    return (
      <div class={{ ...fieldClasses, 'bc-tags-wrap': true }}>
        ...
        <div class="bc-tags-container">
          {tags.map(tag => (
            <span class="bc-tag">{tag}{!this.readonly && !this.disabled && <button type="button" class="bc-tag-remove" onClick={() => this.removeTag(tag)}>&times;</button>}</span>
          ))}
          {!this.readonly && !this.disabled && (
            <div class="bc-tags-input-wrap">
              <input type="text" class="bc-tags-input" placeholder={tags.length === 0 ? this.placeholder : ''} value={this.query} onInput={(e: Event) => this.search((e.target as HTMLInputElement).value)} onFocus={() => this.handleFocus()} onBlur={() => { setTimeout(() => { this.showDropdown = false; }, 200); this.handleBlur(); }} />
              {this.showDropdown && (
                <div class="bc-tags-dropdown">
                  {this.results.map(item => { const val = String(item['name'] || item['id'] || ''); return <div class="bc-tags-option" onMouseDown={() => this.addTag(val)}>{val}</div>; })}
                </div>
              )}
            </div>
          )}
          {this.clearable && tags.length > 0 && !this.disabled && !this.readonly && <button type="button" class="bc-field-clear-btn" onClick={() => this.handleClear()} tabIndex={-1}>&times;</button>}
        </div>
