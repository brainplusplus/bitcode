...
```html
<bc-field-richtext name="myfield" label="My Field" />
```
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| toolbar | string | 'full' | Toolbar mode (full/minimal) |
| min-length | number | 0 | Min content length |
| max-length | number | 0 | Max content length |
